...
import 'package:alba_time/TableInternalWeekViewPage.dart';
...
class TableWeekViewState<T extends Object?> extends State<TableWeekView<T>> {
  ...
  Widget _defaultWeekDayBuilder(DateTime date) {
    final weekTitles = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
  ...
  Widget _defaultWeekNumberBuilder(DateTime date) {
    ...
    final weekNumber = (date.difference(DateTime.now()).inDays / 7).floor() + 1;
